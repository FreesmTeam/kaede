...
export type GlobalsApi = Readonly<{
  "cacheLauncherVersion": typeof import(
    "@/lib/globals/scopes/cache-launcher-version.ts"
  )["cacheLauncherVersion"];
  "cachePathJoin": typeof import(
    "@/lib/globals/scopes/cache-path-join.ts"
  )["cachePathJoin"];
  "declareGlobals": () => void;
  "getLaunchCount": typeof import(
    "@/lib/globals/scopes/get-launch-count.ts"
  )["getLaunchCount"];
  "registerComponent": typeof import(
    "@/lib/globals/scopes/register-component.ts"
  )["registerComponent"];
  "revokeExtensionGlobals": () => void;
}>;

export function declareGlobalsWithApi(Globals: GlobalsApi): void {
  Reflect.set(window, "__KAEDE_INTERNALS__", GlobalInternals);
  Reflect.set(window, "__KAEDE__", GlobalObject);
